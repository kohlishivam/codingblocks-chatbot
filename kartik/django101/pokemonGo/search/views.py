...
def srchlistjs(request):
	...
	context_dict['result_arr'] = search_pokemon("*",
		result_type='dict',from_database=True)
...
def edit(request,pokemon_id):
	if request.method == 'POST':
		...
		return redirect('/edit/%s'%(pokemon_id))
		# return HttpResponse('SAVED %s of type %s and imageurl %s'%(pokemon_name,
		# 	pokemon_type,pokemon_image))
	...
	for pokemon in Pokedex.objects.all():
		if pokemon_id == str(pokemon.id):
			matching_pokemon = pokemon 
	context_dict['matching_pokemon'] = matching_pokemon

	matching_pokemon2 = Pokedex.objects.get(id=pokemon_id)
	context_dict['matching_pokemon2'] = matching_pokemon2
...
def index(request):
	...
	context_dict['iter_arr'] = range(max(1,page_id-3),page_id+4)

	#print len(data_arr[(page_id-1)*n:(page_id+1)*n])
	#print (page_id-1)*n,(page_id+1)*n
